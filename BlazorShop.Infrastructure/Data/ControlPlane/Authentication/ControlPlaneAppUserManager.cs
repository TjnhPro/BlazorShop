namespace BlazorShop.Infrastructure.Data.ControlPlane.Authentication
{
    ...
    public sealed class ControlPlaneAppUserManager : IAppUserManager
    {
        ...
        public ControlPlaneAppUserManager(
        ...
        public async Task<UserLoginResult> LoginUserAsync(AppUser user)
        {
            ...
            var result = await this.signInManager.CheckPasswordSignInAsync(currentUser, user.PasswordHash!, lockoutOnFailure: true);
        ...
        {
            var user = await this.dbContext.Users.FirstOrDefaultAsync(u => u.Email == email);
            if (user is null)
            {
                return 0;
            }
