...
try {
    ...
    $metadataLines = @(
        ...
        "workspaceLayoutVersion: starter-first-sibling-wasm-1",
        "workspaceRoot: $(Get-PortableRelativePath -BasePath $repoRoot -TargetPath (Join-Path $outputRootPath $projectName))",
        "serverProjectRoot: $projectName",
        "wasmProjectRoot: $projectName.WASM",
