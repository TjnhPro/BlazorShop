...
public sealed class AgentHandoffAssembler
{
    ...
    private static string WriteTask(
        ...
        "# Storefront Visual Handoff Task" + Environment.NewLine + Environment.NewLine +
        $"Project: `{project.ProjectId}`" + Environment.NewLine +
        $"Objective: implement generated storefront visual files from the reviewed handoff artifacts only." + Environment.NewLine +
        $"Readiness passed: `{readiness.Passed}`" + Environment.NewLine + Environment.NewLine +
        "Allowed file areas:" + Environment.NewLine +
        string.Join(Environment.NewLine, allowed.Paths.Select(path => $"- `{path}`")) + Environment.NewLine + Environment.NewLine +
        "Protected file areas:" + Environment.NewLine +
        string.Join(Environment.NewLine, protectedFiles.Paths.Select(path => $"- `{path}`")) + Environment.NewLine + Environment.NewLine +
        "Constraints:" + Environment.NewLine +
        "- Preserve Presentation-owned route, BFF, SEO, media, and action behavior." + Environment.NewLine +
        "- Keep screenshots, crops, and reference assets as evidence references; do not copy them as production assets without human review metadata." + Environment.NewLine +
        "- StorefrontBuilder must not consume this package until a later approved Phase 4 plan." + Environment.NewLine +
        "- Expected QA: run the Phase 3C gate and generated storefront proof commands documented in this repo." + Environment.NewLine;
