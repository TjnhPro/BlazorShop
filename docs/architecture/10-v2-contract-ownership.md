...
- Reusable Storefront component mode libraries are split by render/runtime ownership: `BlazorShop.Storefront.Components.Ssr`, `BlazorShop.Storefront.Components.Hybrid`, and `BlazorShop.Storefront.Components.WasmHost`. SSR may consume prepared Presentation context and must not use browser/runtime/client/API transports; Hybrid may bridge server-prepared state to a WasmHost child but must not reference Browser directly; WasmHost may consume Browser controllers but must not reference Presentation or call backend APIs directly.
