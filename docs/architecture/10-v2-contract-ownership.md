...
- Storefront component contracts are presentation contracts, not public HTTP API contracts. Use `BlazorShop.Storefront.Components/Contracts/*` for stable render/input models and `Headless/*` for browser-safe state/behavior contracts. Same-origin browser transport, request DTO construction, mutation state, and cart/checkout/account controllers belong in `BlazorShop.Storefront.Browser`. Store-specific visual templates are host-owned; Storefront V2 interactive root components that hydrate in the browser live in its WASM client assembly and delegate to Browser controllers.
...
`BlazorShop.Storefront.Components/Headless/*` is the preferred home for browser-safe presentation state and action/event contracts that can be reused without Storefront V2 visual markup. `BlazorShop.Storefront.Browser` is the preferred home for browser-side controllers that execute same-origin local API calls and own request DTO creation.
