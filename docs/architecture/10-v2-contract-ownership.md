...
- Reusable Storefront component mode libraries currently exist as `BlazorShop.Storefront.Components.Ssr`, `BlazorShop.Storefront.Components.Hybrid`, and `BlazorShop.Storefront.Components.WasmHost`, but their descriptors describe semantic render/runtime classification only. `Ssr` means server/static/prerender-capable surfaces; `Hybrid` means server-produced or prerendered HTML/page snapshots plus client-side WebAssembly interactivity after hydration; `WasmHost` means browser-side WebAssembly interactive roots that use Browser controllers. H1 removed descriptor/project ownership coupling. H2 proved that semantic `Hybrid` components may physically live in the downloadable WasmHost graph through `StorefrontHybridRuntimeProbe`, while `Components.Hybrid` remains transitional compatibility for the historical contact bridge until H3.
