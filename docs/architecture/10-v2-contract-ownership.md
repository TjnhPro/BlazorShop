...
- Reusable Storefront component mode libraries currently exist as `BlazorShop.Storefront.Components.Ssr`, `BlazorShop.Storefront.Components.Hybrid`, and `BlazorShop.Storefront.Components.WasmHost`, but their descriptors describe semantic render/runtime classification only. `Ssr` means server/static/prerender-capable surfaces; `Hybrid` means server-produced or prerendered HTML/page snapshots plus client-side WebAssembly interactivity after hydration; `WasmHost` means browser-side WebAssembly interactive roots that use Browser controllers. The physical project graph, `Components.Hybrid` role, and descriptor/project consistency tests must be revisited in H1.
