...
Phase 4.11 final closure is stricter than the early skeleton proof. Skeleton/static fixture proof may validate a handoff project shell with seeded files and planned placeholders, but it is not release closure. Release closure must start from a clean unchanged `HEAD`, validate tracked fixture input under `tools/BlazorShop.AI.StorefrontBuilder/tests/generation/fixtures/phase4-11-closure/`, generate fresh disposable pilot output under `obj/storefront-builder/generated/`, require the complete visual artifact chain, run automatic changed-file detection, run runtime visual QA against a generated host, run the Reference visual QA contract, run `FoundationFunctionalFast` at minimum, run regeneration/no-op ownership proof, and finish with the same clean `HEAD`. Full fixture commerce proof through `run-storefront-builder-full-proof-with-fixture.ps1` is the release-level commerce proof when the fixture runtime is available.
...
Before closing Phase 4 visual-skill work, the MVP gate must prove the generated visual plan/implementation/QA path against a handoff project, and the final closure gate must pass from a clean unchanged `HEAD`. The final closure gate regenerates its pilot from tracked fixture input and ignored fresh output, so it must not rely on a pre-existing `obj` project or handoff package. GitHub Actions are not required for this local closure.
