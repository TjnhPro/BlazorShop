...
ReverseEngineering handoff artifacts are prepared for later StorefrontBuilder consumption, but that consumption is not active in Phase 3A, Phase 3B, or Phase 3C. Phase 3C final handoff packages under `analysis/agent-handoff/` are the only approved future generator input shape. Current StorefrontBuilder generation still uses its existing StorefrontBuilder artifacts and scripts.
