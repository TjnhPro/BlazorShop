...
- [x] File path.
- [x] Ownership: generated, managed, protected, external.
- [x] Action: create, replace, patch, skip.
- [x] Source artifact IDs.
- [x] Expected slot.
- [x] Validation rule IDs.
- [x] Conflict behavior.

Rules:

- [x] Protected files cannot have create/replace/patch action.
- [x] Existing manual files cannot be replaced unless marked generated by previous manifest.
- [x] New project must be generated from Starter first, then visual files are applied.
- [x] Re-generation must compare source/spec hash and generated hash.

Output:

- [x] `generation-plan.yaml`

Done when:

- [x] Every generated change is known before file edits.

Verification:

- [x] Dry-run mode prints the file plan.
- [x] Validator fails if a protected file is scheduled for edit.
