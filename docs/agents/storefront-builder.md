...
Phase 4.12 final closure cannot rely on pre-existing `obj` artifacts. The final gate owns the tracked portable handoff fixture at `tools/BlazorShop.AI.StorefrontBuilder/tests/generation/fixtures/phase4-11-closure/portable-handoff`, deletes stale generated pilot output, regenerates a fresh disposable pilot under `obj/storefront-builder/generated/`, and must use StorefrontBuilder-generated `generation-plan.json` plus `agent-task-package/manifest.json`. Do not manually write task packages, manually write generation plans, accept marker-only handoff folders, copy seeded `visual-qa-report.json` files, or treat skeleton/static fixture proof as final closure. Runtime closure requires current-run `visual-qa-runtime-summary.json` binding by operation ID, base URL, screenshot paths, and timestamps before Reference QA is materialized. The final gate records changed-file evidence, runs runtime visual QA, runs the MVP gate, runs `FoundationFunctionalFast` minimum, runs regeneration ownership proof, and verifies the same clean `HEAD` at the end.
