...
Phase 4.11 final closure cannot rely on pre-existing `obj` artifacts. The final gate owns its tracked fixture input, deletes stale generated pilot output, regenerates a fresh disposable pilot under `obj/storefront-builder/generated/`, writes deterministic plan/task-package artifacts, records changed-file evidence, runs runtime visual QA, runs Reference visual QA contract checks, runs `FoundationFunctionalFast` minimum, runs regeneration ownership proof, and verifies the same clean `HEAD` at the end. Skeleton/static fixture proof is useful early feedback only; it is not final closure.
