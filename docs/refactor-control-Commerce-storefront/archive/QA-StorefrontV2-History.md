...
- [x] Run local V2 Playwright account regression: authenticated profile update, invalid password validation, address create/edit/delete, empty and populated order list, order detail, and receipt navigation. 2026-08-11: visible Playwright verified profile persistence/restore, invalid plus rotate-back password flows, address create/edit/default/delete cleanup, the populated ten-order list, detail, receipt, unknown reference protection, navigation, desktop, and mobile. The fixture had orders, so an empty-list fixture was not applicable.
- [x] Verify browser traffic remains same-origin BFF/static/media only, with no direct Commerce Node Storefront calls, no real console/page errors, and no unexpected 5xx responses. 2026-08-11: resource audit returned no cross-origin, direct Commerce, `api/storefront/stores/*`, or `/_blazor` resources; page-error listener returned none. Expected negative 400/404 BFF validation/not-found responses were excluded from unexpected console errors; all positive flows returned 2xx.
...
- [x] V2F6 focused build/tests passed. 2026-07-25: `dotnet build BlazorShop.sln` passed with known MessagePack/Browserslist warnings; focused `StorefrontCommerceFlowCutoverTests|CartCorePhase0InventoryTests|SecurityPrivacyPhase1CsrfTests|StorefrontV2WASMRuntimeFoundationTests` passed 36/36.
...
- [x] Storefront V2 Shared Platform Functional MVP V2F7 checkout/payment runtime cutover keeps browser checkout mutations on same-origin `/api/checkout/*`. 2026-07-25: active `IStorefrontCheckoutClient` and `IStorefrontPaymentClient` resolve generated adapters backed by `IStorefrontRuntimeCheckoutFacade` and `IStorefrontRuntimePaymentFacade`; the V2 host keeps checkout session, cart token flow, idempotency, antiforgery, redirects, and browser DTO mapping.
- [x] V2F7 focused build/tests passed. 2026-07-25: `dotnet build BlazorShop.sln` passed with known MessagePack/Browserslist warnings; focused `StorefrontCommerceFlowCutoverTests|StorefrontV2WASMRuntimeFoundationTests|SecurityPrivacyPhase1CsrfTests|StorefrontCheckout` passed 87/88 with 1 skipped.
- [x] V2F7 Playwright checkout COD flow passed on `http://localhost:18598`. 2026-07-25: evidence saved to `output/playwright/v2f7-checkout-cod-flow-evidence.json` and `output/playwright/v2f7-checkout-cod-flow.png`; verified login, cart add, checkout start, saved billing/shipping address, shipping selection, COD payment selection, review, double-click place order with exactly one POST, cart clearing, and order list/detail for `ORD-20260725-FAE82317`.
- [x] V2F7 browser network guard passed. 2026-07-25: Playwright request capture found zero browser requests to `http://localhost:5180`, no 5xx responses, and no unexpected console/page errors; expected Blazor WASM fetch abort noise during navigation was filtered consistently with existing release QA.
- [x] Storefront V2 Shared Platform Functional MVP V2F8 address/consent runtime cutover keeps auth-sensitive account policy in V2 host. 2026-07-25: active `IStorefrontAddressClient` and `IStorefrontConsentClient` resolve generated adapters backed by `IStorefrontRuntimeAddressFacade` and `IStorefrontRuntimeConsentFacade`; `StorefrontSessionResolver`, `StorefrontAuthClient`, and protected `IStorefrontCustomerClient` calls remain documented V2-owned bearer-token exceptions.
- [x] V2F8 focused build/tests passed. 2026-07-25: `dotnet build BlazorShop.sln` passed with known MessagePack/Browserslist warnings; static/client account/address/consent tests passed 42/42, selected host smoke tests passed 8/8, and guest completion token guard tests passed 71/71.
- [x] V2F8 Playwright account/consent flow passed on `http://localhost:18598`. 2026-07-25: evidence saved to `output/playwright/v2f8-account-consent-flow-evidence.json` and `output/playwright/v2f8-account-consent-flow.png`; verified register allowed, password recovery sent state, login/logout, profile edit, address add/edit/default/delete through same-origin BFF, order paging/detail, cross-customer order denial, and consent save/revoke.
...
- [x] SRH9 Storefront Runtime release gate passed. 2026-07-26: Client, Runtime, Components, WASM, V2, and Starter builds passed; Runtime focused tests passed `51/51`; generated client tests passed `16/16`; Architecture/Storefront boundary gate passed `781/783` with `2` existing skipped cart service tests. Starter package-boundary guard now restores into an isolated repo-local NuGet cache so stale global `1.0.0-local` Runtime packages cannot mask current package output.
- [x] CLH7 browser/local BFF structured error checks passed. 2026-07-26: `StorefrontLocalApiResult<T>` now exposes structured `Error` details with status/code/trace/field-errors/retryable while keeping `Message` compatibility; local API/cart error bodies include browser-safe structured semantics; focused `StorefrontV2WASMRuntimeFoundationTests|StorefrontBffBoundaryHardeningTests|StorefrontCommerceFlowCutoverTests` passed `34/34`.
- [x] CLH8 StorefrontBuilder/Starter component guidance guard passed. 2026-07-26: docs and tests state generated storefronts may consume `Contracts`, `Headless`, and `Browser` primitives, but must not import/copy retired `Storefront.Components/Features` wrappers. Focused package/Builder/component docs tests passed `51/51`.
- [x] CLH9 Storefront Components Logic Only release gate passed. 2026-07-26: Components, WASM, Storefront V2, and Starter builds passed with `0` warnings; focused component/WASM/BFF/package/Builder tests passed `77/77`; source scans found no forbidden `Features` imports in Headless/Starter/StorefrontBuilder and no remaining shared hardcoded V2 route/copy strings. StorefrontBuilder generated proof and isolation gate passed after fixing local `1.0.0-local` package cache handling. Browser QA passed order-email COD/account flow, registration policy, product selection/add-to-cart same-origin BFF probe, gallery thumbnail switch, and structured local BFF validation/auth error response checks with no direct Commerce Node browser calls.
- [x] SCR10 Storefront Components Compatibility Removal docs/static QA passed. 2026-07-26: current architecture and StorefrontBuilder docs now state `Storefront.Components` only owns `Contracts`, `Headless`, and `Browser`; V2 host/WASM client owns cart, checkout, and account interactive markup/CSS/layout, while V2 owns product, catalog, and deals markup/CSS/layout; Runtime compatibility aliases are removed. Focused final-boundary tests passed `144/144`.
- [x] SCR12 browser release QA passed. 2026-07-26: `scripts/qa/run-storefront-order-email-e2e.ps1 -Headless` passed for login, add-to-cart, checkout COD, order confirmation, account order list/detail/receipt, order email, queued retry behavior, and same-origin network guard. `scripts/qa/run-storefront-registration-policy-e2e.ps1 -Headless` passed. Additional Playwright probe `.gstack/qa-reports/scr12-browser-qa/result.json` passed for home, `/category/apparel`, search, product detail/purchase, add-to-cart, cart quantity update/remove/clear, and checkout empty state.
- [x] SCR12 browser console/static asset QA passed. 2026-07-26: initial failure showed `Root component type 'BlazorShop.Storefront.Components.Account.StorefrontAccountApp' could not be found in the assembly 'BlazorShop.Storefront.V2'`; fix moved cart/checkout/account interactive root components to `BlazorShop.Storefront.V2.WASM/Components/*` while V2 hosts the WASM assembly. Passing probe verified `storefrontWasmInterop.js` returns HTTP 200 `text/javascript`, browser console has no JS/.NET hydration/static module errors, anti-forgery-backed same-origin cart actions submit, and no direct browser calls to Commerce Node were recorded.
- [x] SPF14 Storefront Presentation documentation boundary updated. 2026-07-27: current architecture, agent, StorefrontBuilder, visual reverse engineering, ADR, and QA docs now state `BlazorShop.Storefront.Presentation` owns shared App/Routes/page services/BFF/SEO/media composition; Storefront V2 is the production host/visual implementation; Starter/generated storefronts consume Presentation as a package and provide views/assets/copy instead of recreating application logic.
- [x] SPF17 Storefront Presentation adapter ownership cutover passed. 2026-07-27: neutral catalog/content, cart, checkout, address, payment, auth/session/customer, current-store, display-context, and price-format defaults now live in `BlazorShop.Storefront.Presentation`; V2 no longer registers generated adapters for Presentation contracts; Starter home renders Presentation `StorefrontHomePageContext` without `StorefrontBootstrapService`. Builds passed for Presentation, Runtime, V2, and Starter. Focused SPF17 tests passed `51/54` with `3` future-phase skips; touched guardrails passed `64/65` with `1` existing skip; Starter/cutover architecture tests passed `27/30` with `3` future-phase skips.
- [x] F1.85 Browser boundary re-entry QA passed. 2026-07-29: local V2 runtime `http://localhost:18598` passed Playwright cart re-entry, checkout re-entry after cart mutation, account identity reset between generated users in one browser session, checkout submit disable guard, cart transport failure recovery, Browser-owned `storefrontWasmInterop.js` static asset HTTP 200, zero direct Commerce Node browser calls, and zero console warnings/errors. `scripts/qa/run-storefront-order-email-e2e.ps1 -Headless` also passed COD order/email/retry evidence for `ORD-20260729-4CE10AF3`.
...
- [x] Product Summary primitives extraction QA passed. 2026-08-10: `BlazorShop.Storefront.Components.Primitives` owns browser-safe render-only Product Summary image, purchase action, and card primitives while referencing only `BlazorShop.Storefront.Components`; V2 and V2.WASM own final labels/classes. Focused builds passed for Components, Components.Primitives, Components.Ssr, Components.WasmHost, V2.WASM, and V2. Focused tests passed Product Summary primitive, visual neutrality, render-mode ownership, ProductRail, and dependency guardrails. Playwright QA on `http://localhost:18598` verified home latest products, category listing, search results, V2.WASM discounted rail loading/success/empty/error/retry states, primitive rail items, and direct add-to-cart through exactly one same-origin `/api/cart/lines` POST with no direct Commerce/Control Plane/Storefront API browser request and no server UI circuit.
...
- [x] `BlazorShop.Storefront.Components.Primitives` is a separate render-only Razor primitive package, not a component mode. It references only `BlazorShop.Storefront.Components` and must not own descriptors, final visual classes/copy/layout, CSS/assets, Browser/Presentation/Runtime/Client/backend references, API calls, JS interop, or `@rendermode`.
...
- [x] `dotnet build BlazorShop.PresentationV2/BlazorShop.Storefront.V2.WASM/BlazorShop.Storefront.V2.WASM.csproj`. 2026-07-13: passed.
...
- [x] Phase 0 browser project dependency boundary is guarded. 2026-07-23: `StorefrontPageCompositionGuardrailTests.StorefrontBrowserProjects_KeepPortableDependencyBoundary` asserts `Storefront.Components` and `Storefront.V2.WASM` do not reference Application, Domain, Infrastructure, Control Plane, or Commerce Node API projects, and that WASM references Components.
- [x] Phase 0 made no runtime behavior changes; only tests and QA/planning docs changed.
- [x] Phase 1 moved Storefront route files under `Pages/Ssr`, `Pages/Hybrid`, and `Pages/WasmHost` without changing `@page` declarations. 2026-07-23: source-reference tests and `StorefrontPageCompositionGuardrailTests.PageInventory_RecordsCurrentRenderingOwnershipBaseline` were updated to the new paths.
- [x] Phase 1 architecture guidance records the render-ownership folder convention. 2026-07-23: `docs/architecture/05-project-and-folder-guide.md` and `Pages/README.md` describe `Ssr`, `Hybrid`, and `WasmHost`.
- [x] Phase 2 consolidates account route shells into one server host. 2026-07-23: `AccountHostPage.razor` declared `/account` and `/account/{*Path}` as the migration host. 2026-07-27 SPF11: `Storefront.Presentation/Pages/WasmHost/Account/AccountRoutePage.razor` owns the routes, noindex/private metadata, session redirect, return URL, and antiforgery token bootstrap; V2 and Starter now provide account views through foundation view registration.
- [x] Phase 2 moves account navigation/shell responsibility into portable Storefront components. 2026-07-23: `Storefront.Components/Features/Account/AccountApp.razor` and `AccountNavigation.razor` own profile, addresses, orders, order detail/receipt, and change-password sub-routes while reusing the existing account feature panels. 2026-07-26 SCR10: this was a migration state; account markup is now V2-owned and shared `Features` wrappers are retired.
...
- [x] Phase 3 removes duplicate first-load fetches for server snapshots. 2026-07-23: `CartView` and `CheckoutShell` skip initial browser GET when `DataMode=InitialSnapshot`; account panels can also skip GET when an initial snapshot is explicitly supplied.
- [x] Phase 3 verification passed: Components build 0 warnings and focused Storefront V2 WASM/static guardrails passed 77/77.
- [x] Phase 4 establishes the `Storefront.Components/Features/*` convention. 2026-07-23: account, cart, and checkout feature components live under `Features/Account`, `Features/Cart`, and `Features/Checkout`; `Features/README.md` documents placement rules. 2026-07-26 SCR10: convention retired; `Storefront.Components` now exposes only `Contracts`, `Headless`, and `Browser`.
...
- [x] Phase 5 extracts portable catalog/deals display components without a new API. 2026-07-23: `Features/Catalog/ProductSummaryGrid`, `ProductSummaryCard`, and `Features/Deals/DealsBlock` render presentation-only `ProductSummaryItem` data mapped by Storefront V2 pages. 2026-07-26 Storefront Components Compatibility Removal SCR4: those shared visual wrappers are retired; Storefront V2 now owns `StorefrontProductSummaryGrid`, `StorefrontProductSummaryCard`, and `StorefrontDealsSection` markup.
...
- [x] Phase 0 baseline confirmed WASM was already wired through `AddInteractiveWebAssemblyComponents()` and `AddInteractiveWebAssemblyRenderMode()`, while real cart/account/checkout interactions still used server forms and/or `storefrontCommerce.js` at that time. 2026-07-17: source review before Phase 1 code changes; F1.47/F1.52 moved core browser transport into Presentation and kept `storefrontCommerce.js` visual-only.
...
- [x] COD order email E2E passes through Storefront browser, CommerceNode queued email, and Mailpit capture. 2026-07-18 Email SMTP Capture Phase 6: `.\scripts\qa\run-storefront-order-email-e2e.ps1` passed with visible Chromium; order `ORD-20260718-92E28ABB` produced exactly one captured `order.placed` email from `default-sender@example.local`.
- [x] Storefront order placement still succeeds when SMTP delivery is unavailable. 2026-07-18 Email SMTP Capture Phase 6: same runner disabled store SMTP, placed `ORD-20260718-01F9A3A5`, observed queued-message retry state, restored SMTP, and admin retry sent the message.
...
- [x] WASM same-origin client has no Commerce Node base URL, node key, node secret, refresh token, or access token configuration. 2026-07-17 Phase 2: WASM registers `HttpClient` with `builder.HostEnvironment.BaseAddress`; `StorefrontV2WASMRuntimeFoundationTests` rejects absolute/protocol-relative routes.
- [x] Cart WASM loads existing cart. 2026-07-17 Phase 3: `/my-cart` prerenders `CartView` from server snapshot and the component refreshes via `GET /api/cart`; focused cart suite passed 25/25.
- [x] Cart WASM updates quantity. 2026-07-17 Phase 3: `CartView` calls same-origin `PUT /api/cart/lines/{lineId}` with CSRF-backed local API client; focused cart suite passed.
- [x] Cart WASM removes a line. 2026-07-17 Phase 3: `CartView` calls same-origin `DELETE /api/cart/lines/{lineId}`; focused cart suite passed.
- [x] Cart WASM clears cart. 2026-07-17 Phase 3: `CartView` calls same-origin `DELETE /api/cart`; focused cart suite passed.
...
- [x] Checkout WASM blocks empty cart. 2026-07-18 Phase 5: `/api/checkout` returns a browser checkout empty state when cart token/cart lines are missing and `CheckoutShell` renders it on `/checkout`.
...
- [x] Checkout WASM completes happy-path order placement. 2026-07-18 Phase 5: `CheckoutShell` can call same-origin `/api/checkout/place-order`; Storefront V2 server keeps idempotency/version validation, provider redirect handling, and cart-cookie clearing authoritative.
...
- [x] Headless Storefront Foundation F5.3 removes Application auth DTOs from Storefront V2 while preserving refresh-cookie and account BFF behavior. 2026-07-24: Storefront V2 build passed with 0 warnings; `StorefrontV2AuthClientTests` passed `10/10`; reset/register/change-password host smoke subset passed `3/3`; account profile/address/order/change-password subset passed `8/8`. Known warnings were existing MessagePack NU1902/NU1903 and Browserslist notices.
...
- [x] Product detail reads Storefront product `mediaGallery` and keeps legacy `Image` fallback. 2026-07-20 Product Image Gallery Phase 2/3: `StorefrontV2ApiClientTests.GetPublishedProductBySlugAsync_ReadsDeliveryMetadata` and `StorefrontBrandingMarkupTests.ProductPage_RendersProductImageGalleryComponent` passed.
- [x] Product detail gallery frames are locked to 1:1 for main image, thumbnails, and no-image placeholder. 2026-07-20 Product Image Gallery Phase 4: `StorefrontBrandingMarkupTests.ProductGalleryCss_EnforcesSquareImageFrames` passed.
- [x] Product detail loaded media is not covered by hidden fallback placeholders. 2026-07-22 investigation: Chromium confirmed product media responses `200 image/webp`, main image `naturalWidth=570`, placeholder `display:none`, and `StorefrontBrandingMarkupTests.ProductGalleryCss_EnforcesSquareImageFrames` guards the `[hidden]` override.
- [x] Product detail thumbnails render as image cells instead of collapsed dots. 2026-07-22 investigation: Chromium measured thumbnail buttons at about `80x80` after the CSS fix; `site.css` did not contain the newly used Tailwind `h-20/w-20/sm:*` utilities, so `storefront.css` now owns gallery thumbnail dimensions.
- [x] Product gallery carousel hardening baseline captured the desktop thumbnail shrink regression. 2026-07-22 Phase 0: product page returned `200` with SSR gallery markup, media returned `200 image/webp`, Playwright measured main frame `542x542`, 14 desktop thumbnails collapsed to about `28x28`, mobile thumbnails measured `80x80`, thumbnail click changed the main image, and screenshots were saved under `output/playwright/storefront-product-gallery-phase0-*.png`.
- [x] QA seed has a default product gallery fixture with at least 3 stored media rows and one primary image without reseeding runtime store config on every start. 2026-07-20 Product Image Gallery Phase 5: `CommerceNodeProductGalleryReadServiceTests.DevelopmentSeeder_KeepsIncrementalThreeImageGalleryFixture` guards incremental fixture creation.
- [x] Visible browser QA for product gallery thumbnail click, mobile strip layout, and broken-image detection remains covered for current gallery hardening scope. 2026-07-22 Phase 5: local V2 runtime returned Storefront `/health=200` and CommerceNode `/api/commerce/healthz=200`; Playwright desktop measured main frame `542x542` and 14 thumbnails at `80x80`; mobile measured main frame `308x308` and thumbnails at `80x80`; next/previous buttons changed images and disabled at boundaries; middle thumbnail changed the main image; selected thumbnail stayed inside the scrolling viewport; placeholder stayed `display:none`; ArrowRight moved selection to index `1` and ArrowLeft returned to index `0`; screenshots and JSON evidence were saved under `output/playwright/storefront-product-gallery-phase5-*`.
- [x] Product gallery carousel hardening release gate passed. 2026-07-22 Phase 6: focused Storefront suite passed `217/217`; full `BlazorShop.Tests.V2` passed `1267`, skipped `2`; post-test Playwright release rerun kept desktop main frame `542x542`, mobile main frame `308x308`, 14 thumbnails at `80x80`, selected thumbnail in viewport, boundary disabled states correct, and placeholder `display:none`. Known warnings were existing MessagePack NU1902/NU1903 and Browserslist notices.
- [x] Product gallery broken-image fallback works repeatedly after carousel changes. 2026-07-22 post-review QA: Playwright forced two broken main image URLs with a valid next-image selection between them; fallback displayed both times even after inline `onerror` became null, main frame stayed `542x542`, and a broken thumbnail showed its fallback while the thumbnail cell stayed `80x80`. `node --check`, focused Storefront gallery static test, and full `BlazorShop.Tests.V2` (`1267` passed, `2` skipped) passed after the fix. Evidence: `output/playwright/storefront-product-gallery-broken-fallback.json` and `.png`.
- [x] Product gallery navigation controls do not show a visible image counter and are centered on the left/right sides of the main image. 2026-07-22 investigation: root cause was `sr-only` not being available in the emitted CSS and the counter being a third flex child, which pushed the next button toward the center. Playwright confirmed `visibleCounterText=false`, no `data-storefront-gallery-status`, previous/next button vertical delta `0`, icon delta `0`, and both controls 12px from the main image edges. Evidence: `output/playwright/storefront-product-gallery-control-fix.json` and `.png`.
- [x] Product detail gallery is a render-only `Components.Primitives/Product/StorefrontProductGallery` primitive fed by `ProductGalleryItem`, `ProductGalleryLabels`, `ProductGalleryClasses`, and `ProductGalleryState`; V2 owns final labels/classes and JS progressive enhancement. 2026-07-23 Rendering Components Phase 6: Storefront V2 build passed and `StorefrontBrandingMarkupTests.ProductPage_RendersProductImageGalleryComponent` guarded the earlier component boundary. 2026-07-26 Storefront Components Compatibility Removal SCR4: shared `Features/Product/ProductGallery` was retired. 2026-08-10 Phase 3.2 supersedes the temporary V2-owned gallery markup position now that `Components.Primitives` exists as a render-only, browser-safe package.
- [x] Product detail pricing, availability, and informational variant list render through `Components.Ssr/Product` display components over prepared Presentation product views while `StorefrontProductPurchasePanel` remains V2-owned. 2026-08-10 Phase 3.2: focused builds passed for Components, Components.Primitives, Components.Ssr, V2, and V2.WASM; focused tests passed Product Detail display components, primitive dependency, visual neutrality, render-mode ownership, product ownership guardrails, and commerce script guardrails.
- [x] Product detail purchase UI is a portable `ProductPurchasePanel` fed by page-owned variant/sellability snapshots; add-to-cart, selection preview, delivery, stock, quantity, and disabled hard-block markers remain present. 2026-07-23 Rendering Components Phase 6: focused Product/Catalog guardrail tests passed. 2026-07-26 SCR10: product purchase markup is now V2-owned as `StorefrontProductPurchasePanel`.
- [x] Category, search, deals, and new releases compose `ProductSummaryGrid` through `StorefrontProductSummaryMapper` instead of duplicating product summary card logic. 2026-07-23 Rendering Components Phase 6: focused Product/Catalog guardrail tests passed. 2026-07-26 SCR10: catalog/deals markup is now V2-owned through `StorefrontProductSummaryGrid`, `StorefrontProductSummaryCard`, and `StorefrontDealsSection`.
- [x] Storefront rendering component release QA passed for home, category, search, today's deals, new releases, and product detail. 2026-07-23 Rendering Components Phase 8: local V2 runner returned Storefront `/health=200`; Playwright verified all sampled pages returned 200, reusable deals/product summary markers rendered, product gallery desktop main frame `542x542`, mobile main frame `308x308`, no visible gallery counter, product purchase panel/selection preview markers present, no console errors, and no failed Storefront responses. Evidence: `output/playwright/storefront-rendering-components-phase8.json`, `storefront-rendering-components-phase8-product-desktop.png`, and `storefront-rendering-components-phase8-product-mobile.png`.
- [x] Storefront rendering route smoke passed for cart, checkout, payment success/cancel, content page, search noindex, and unauthenticated account redirect. 2026-07-23 Rendering Components Phase 8: Playwright verified the sampled routes returned 200 where expected, `/search?q=shirt` emitted `noindex,follow`, `/account/profile` redirected to `/signin?returnUrl=%2Faccount%2Fprofile`, and there were no console errors or failed Storefront responses. Evidence: `output/playwright/storefront-rendering-components-phase8-routes.json`.
- [x] Storefront rendering component focused automated gate passed. 2026-07-23 Rendering Components Phase 8: Storefront V2 build passed; focused Storefront tests passed `167/167`; production release smoke passed ControlPlane API/Web health, CommerceNode API health, Storefront health, CommerceNode Storefront/CommerceAdmin Swagger, and Nginx unknown-host `403`. Known warnings were existing MessagePack NU1902/NU1903 and Browserslist notices.
...
- [x] Visible browser QA confirms unmanaged-stock product can be added to cart. 2026-07-26: Playwright probe added `qa-unmanaged-stock-product` to cart successfully after selection preview and did not show the false `This product is out of stock.` guard; evidence: `output/playwright/investigate-storefront-checkout-product-account.json`.
- [x] Visible browser QA confirms managed out-of-stock product cannot be added to cart. 2026-08-10 Phase 3.2: Playwright verified `qa-out-of-stock-product` rendered a disabled purchase submit button and sent no cart mutation when forced-clicked.
...
- [x] Published Storefront page public contract exposes template identity without admin fields. 2026-07-23 Phase 1: `StorefrontPagePublicDto` appends nullable `PageKey`, `StorefrontPageService.GetPublishedBySlugAsync` maps it, Storefront V2 client reads `pageKey`, and OpenAPI contract/snapshot tests prove `StorefrontPagePublicDto` does not expose IDs, store ownership, publish/archive state, sitemap, navigation, or display-order fields.
- [x] Storefront content template catalog includes FAQ and customer-service page keys aligned to `/pages/faq` and `/pages/customer-service`. 2026-07-23 Phase 2: `StorefrontPageContentRules.PageKeys` and `StorefrontPageTemplateCatalog` include `faq` and `customer_service`, tests reject `generic/contact_form` page types, and Development seeding creates missing FAQ/customer-service fixtures without overwriting existing matching pages.
- [x] `/pages/{slug}` renderer is template-aware without adding dedicated content routes. 2026-07-23 Phase 3: `StorefrontPagePresentationResolver` maps known `PageKey` values to standard/policy/FAQ/support presentation, `StorefrontPage.razor` passes structured data into `SeoHead`, FAQ uses `WebPage` JSON-LD until structured FAQ entries exist, and focused resolver/composition/structured-data tests passed 58/58.
- [x] Account routes share a route shell without moving account behavior into the shell. 2026-07-23 Phase 4: historical implementation used `AccountPageShell`; current rendering refactor supersedes it with `AccountHostPage` plus `AccountApp`.
- [x] Storefront V2 route pages are grouped by ownership without route URL changes. 2026-07-23 Phase 5: pages moved under `Pages/Hybrid/Catalog`, `Pages/Ssr/Content`, `Pages/Hybrid/Commerce`, `Pages/Ssr/Auth`, `Pages/WasmHost/Account`, and `Pages/Ssr/System`; code-behind namespaces and source-reference tests were updated, and `docs/architecture/05-project-and-folder-guide.md` records the structure.
- [x] Dynamic Storefront page composition release QA passed. 2026-07-23 Phase 6: fixed incremental Development seeding so existing local DBs create missing FAQ/customer-service pages without overwriting edited store content; `CommerceNodeDevelopmentSeederTests` passed 2/2, focused Storefront tests passed 562/562 with 2 skipped, maintenance host smoke matched 2/2 cases, Playwright evidence passed 29/29 route/SEO/browser checks at `output/playwright/storefront-page-composition-phase6-evidence.json`, and supplemental collection/account/auth/payment checks passed 11/11 at `output/playwright/storefront-page-composition-phase6-supplemental-evidence.json`. Full `BlazorShop.Tests.V2` passed 1326 and skipped 2 but failed 1 legacy-removal guardrail because three unrelated legacy-removal files were already deleted in the worktree before this phase.
...
- [x] Head icon metadata uses store favicon/png/apple/MS tile values when configured. 2026-07-15: HTTP HTML checks found configured favicon/png/MS tile metadata after fixing `ISSUE-001` head metadata collision. 2026-08-08: `StorefrontIconHead` in Presentation now owns favicon/png/apple/MS tile tags, while V2 `StorefrontApplicationHead` places it before non-icon `StorefrontBrandHead`.
...
- [x] Headless Storefront Foundation F5.1 reads current-store, public configuration, capability projection, and currency preference through the generated `BlazorShop.Storefront.Client` adapter. 2026-07-24: `StorefrontGeneratedConfigurationClientTests`, current-store/display-context tests, and legacy route-shape client tests passed `51/51`; Storefront V2 build passed with 0 warnings.
- [x] Storefront V2 release validation includes the generated-client regeneration gate before consuming a new `BlazorShop.Storefront.Client` package. 2026-07-26 OpenAPI Client Hardening OCH4: run `scripts/qa/run-storefront-client-regeneration-gate.ps1` before package consumer proof.
- [x] Storefront V2 generated-client package release checklist includes canonical contract comparison, regeneration gate, `Storefront.Client`/Runtime/Storefront V2 builds, package consumer proof, and focused generated-client/Storefront tests. 2026-07-26 OpenAPI Client Hardening OCH7.
...
- [x] Storefront SEO/discovery is shared through `BlazorShop.Storefront.Presentation`. 2026-07-26 SPF5: focused Presentation/Layout/PageComposition/SEO/Host/BFF guardrail suite passed 130/130; V2 and Starter used the shared SEO endpoint mapping. 2026-07-27 SPF12: both hosts now use aggregated `UseStorefrontPresentation()` and `MapStorefrontPresentation()`.
...
- [x] `/js/storefrontCommerce.js` returns JS as the V2 visual script; Presentation `_content/BlazorShop.Storefront.Presentation/js/storefront.application.js` owns core browser transport/security behavior. 2026-07-28 F1.53: V2 build and focused asset tests guard both script surfaces.
...
- [x] Storefront root asset inventory is guarded by automated tests. 2026-07-28 F1.53: `LayoutAssetFoundationTests.StorefrontRoot_DefinesExpectedAssetsWithoutDuplicates` asserts host CSS, `_framework/blazor.web.js`, Presentation `storefront.application.js`, and V2 `js/storefrontCommerce.js` exactly once.
- [x] Storefront V2.WASM CSS static asset presence is guarded. 2026-08-08: `StorefrontV2WASMRuntimeFoundationTests` asserts `wwwroot/css/wasm-site.css` exists, is generated from the V2.WASM Tailwind pipeline, and V2.WASM no longer ships a conflicting `wwwroot/css/site.css`.
- [x] Storefront root stylesheet/script links use static asset resolver/fingerprint-capable URLs. 2026-08-08: host head/script components resolve V2 `css/site.css`, V2.WASM `css/wasm-site.css`, `css/storefront.css`, and `js/storefrontCommerce.js` through Razor `@Assets[...]`; `LayoutAssetFoundationTests` and `StorefrontV2HostSmokeTests` guard the allowlist and order.
- [x] Storefront V2/V2.WASM CSS reproducibility gate is available. 2026-08-08: `scripts/qa/run-storefront-v2-css-reproducibility.ps1` runs project-local `npm ci` and `npm run tailwind:build`, compares regenerated CSS against tracked `site.css` and `wasm-site.css`, and restores tracked bytes after proof generation.
- [x] Storefront V2 Docker publish path builds both Tailwind outputs. 2026-08-08: `V2ProductionReadinessTests` guards the Dockerfile V2/V2.WASM npm build steps, and focused `dotnet publish` proof confirmed `wwwroot/css/site.css`, `wwwroot/css/wasm-site.css`, `wwwroot/css/storefront.css`, and `wwwroot/js/storefrontCommerce.js` are present without root package metadata collisions.
...
- [x] Storefront publish output contains required root/static assets. 2026-07-28 F1.53 keeps the split: framework boot and Presentation `storefront.application.js` load before V2 `wwwroot/js/storefrontCommerce.js`, which remains visual-only.
...
- [x] Storefront V2/WASM CSS fingerprint browser layout QA passed. 2026-08-08: `run-v2-local.ps1 -StopExisting -NoOpenBrowser` started the full V2 stack; Playwright route evidence `output/playwright/storefront-v2-css-phase7-routes-evidence.json` verified fingerprinted `site.css`, `wasm-site.css`, `storefront.css`, and `storefrontCommerce.js` load with 0 static asset failures, 0 direct Commerce Node browser calls, and 0 static/hydration console errors. Same-origin CSS fixture `output/playwright/storefront-v2-css-phase7-evidence.json` measured V2.WASM cart image frame `96px`, checkout responsive grid, and account responsive grid; screenshots saved under `output/playwright/storefront-v2-css-phase5-*.png`.
...
- [x] Browser release gate covers product add-to-cart, cart, checkout entry/order placement, account order pages, email queue behavior, and no direct retired/protected flow calls. 2026-07-28 F1.52/F1.53: `.\scripts\qa\run-storefront-order-email-e2e.ps1 -Headless -TimeoutSeconds 1200` passed with no 5xx responses and no retired flow calls.
...
- [x] Storefront account profile and change-password pages render through local SSR routes and keep mutations behind antiforgery-protected local POSTs. 2026-07-17 Customer Identity Phase 3: `StorefrontV2HostSmokeTests` and `StorefrontV2ApiClientTests|StorefrontV2AuthClientTests` passed inside the 66/66 focused run.
- [x] Storefront account order self-service backend contract is ready for future UI pages. 2026-07-17 Customer Identity Phase 4: CommerceNode scoped `orders/current-user`, detail, and receipt endpoints are Bearer-protected, paged, and safe in OpenAPI; no Storefront V2 order UI page was added in this phase.
- [x] Storefront guest completion backend contract returns safe receipt/detail data only with a token. 2026-07-17 Customer Identity Phase 5: CommerceNode guest lookup remains anonymous but requires reference/token and returns the safe detail DTO; Storefront V2 UI page remains future Phase 6 work.
- [x] Storefront account orders, order detail, receipt, and address pages render through local SSR account routes. 2026-07-17 Customer Identity Phase 6/7: Storefront V2 build passed; account UI hardening subset passed 68/68 after address command forms were added.
- [x] Storefront account UI uses bearer-backed Storefront API client calls without browser-supplied `userId`, `customerId`, `storeId`, roles, statuses, or provider references. 2026-07-17 Customer Identity Phase 6: client tests assert bearer routes for order list/detail/receipt and host smoke tests render safe DTO fields.
- [x] Storefront account address book can create, update, delete, and set default shipping/billing through Address Core without exposing customer/store identity fields. 2026-07-17 Customer Identity Phase 7: `CustomerAddressCommands_SendBearerAndUseScopedRoutes` and `AccountAddresses_PostCreate_UsesBearerAndRedirectsSaved` passed.
- [x] Customer Identity account release gate passed for Storefront V2. 2026-07-17 Phase 7: CommerceNode API build passed, Storefront V2 build passed, focused identity/account suite passed 245/245, and final Storefront account subset passed 68/68.
...
- [ ] Shipping Core visible checkout QA confirms shipping total updates checkout review total.
...
- [x] Headless Storefront Foundation F5.6 consent/captcha/newsletter/recommendations migration remains behavior-stable. 2026-07-24: Storefront V2 build passed 0 warnings; generated configuration, CSRF, endpoint mapping, register-policy, and forgot-password focused run passed 12/12. Known warnings: existing MessagePack NU1902/NU1903 and Browserslist.
- [x] Headless Storefront Foundation F5 final dependency removal remains behavior-stable. 2026-07-24: Storefront V2 build passed 0 warnings after removing the `Application` project reference and Dockerfile backend/core copies; architecture boundary guard now blocks Storefront V2 backend/core references and `Web.SharedV2.Models` business DTO imports. Focused boundary/generated-client run passed 8/8, `Contract|Boundary` slice passed 143/143, WASM runtime foundation passed 17/17, and representative host smoke passed 6/6. The full broad `Storefront|Contract|Boundary` filter exceeded the 5-minute command timeout, so it was split into targeted runs. Known warnings: existing MessagePack NU1902/NU1903 and Browserslist.
- [x] Headless Storefront Foundation F6 runtime/module boundary decision recorded. 2026-07-24: no `Storefront.Runtime` or `Storefront.Features.*` project was created because repeated code remains tied to Storefront V2 host/BFF ownership and there is still one real storefront consumer. Architecture docs kept generated API transport in `Storefront.Client`, portable UI in `Storefront.Components/Features/*`, and route/BFF/SEO/design/deployment in Storefront V2 at that time. 2026-07-26 SCR10: `Storefront.Components/Features/*` has been retired; portable reuse now lives in `Contracts`, `Headless`, and `Browser`. `dotnet test BlazorShop.Tests.V2/BlazorShop.Tests.V2.csproj --no-restore --filter "FullyQualifiedName~Runtime|FullyQualifiedName~Architecture"` passed 79/79 after rerunning without parallel testhost locks. Known warnings: existing MessagePack NU1902/NU1903 and Browserslist.
- [x] Headless Storefront Foundation F7 package/isolation/browser gate passed. 2026-07-24: architecture/production readiness boundary slice passed 16/16; Storefront OpenAPI/generated-client slice passed 46/46; package consumer proof passed 1/1; `scripts/qa/run-storefront-foundation-isolation-gate.ps1` packed `BlazorShop.Storefront.Client.1.0.0-local.nupkg` and separately published Commerce Node API and Storefront V2; `scripts/qa/run-v2-production-release-smoke.ps1` passed health, Swagger, and Nginx unknown-host checks; Playwright release QA passed 13/13. F7 fixed product detail placeholder alt binding by passing the actual product name into `ProductGallery`. Known warnings: existing MessagePack NU1902/NU1903, local package readme advisory, and Browserslist.
- [x] Headless Storefront Foundation F8 Starter readiness decision recorded. 2026-07-24: foundation Definition of Done is complete and `Headless Storefront Platform Foundation.completion.md` records that a separate `BlazorShop.Storefront.Starter` planning phase can begin without reopening backend/client boundary decisions. Starter implementation remains explicitly out of scope for this foundation. F8 architecture/docs guard passed 43/43. Known warnings: existing MessagePack NU1902/NU1903 and Browserslist.
- [x] Storefront Shared Platform V2F9 contract ownership guard passed. 2026-07-25: `StorefrontContractOwnershipTests` now guards `Storefront.Components`, `Storefront.Runtime`, `Storefront.Client`, Storefront V2 `Web.SharedV2.Models` business-contract usage, and the documented manual-client exception list. Solution build passed and focused generated-client/package/ownership slice passed 16/16. Known warnings: existing MessagePack NU1902/NU1903 and Browserslist.
- [x] Storefront Shared Platform V2F10 host composition cleanup passed. 2026-07-25: `StorefrontServiceCollectionExtensions` is split into host options, Runtime, generated-client, BFF dependency, SEO/media, and auth/session/antiforgery groups; `Program.cs` is guarded as composition-only. Solution build passed, static composition/boundary slice passed 9/9, active commerce cutover slice passed 8/8, and Playwright route smoke passed for home/product/cart/checkout/account with no direct browser request to Commerce Node.
- [x] Storefront Shared Platform V2F11 Starter/generated compatibility definition passed. 2026-07-25: architecture, visual reverse engineering, and agent docs now define Starter/generated consumer rules for `Storefront.Client`, `Storefront.Runtime`, optional browser-safe `Storefront.Components`, same-origin BFF actions, generated package contracts, and forbidden Storefront V2/backend references. Solution build passed and focused Starter/StorefrontBuilder/package compatibility slice passed 35/35, including local package proof for Client, Runtime, and Components. Playwright production QA was intentionally deferred to V2F12.
- [x] Storefront Shared Platform V2F12 production browser QA release gate passed. 2026-07-25: registration policy, password recovery, COD order/email, route/resilience, consent, sitemap/robots, account order access, network guardrails, focused runtime/client/component/host tests, and final solution build passed for `Storefront.V2`. Evidence is recorded in `Storefront V2 Shared Platform Functional MVP.qa.md`; browser runs observed no direct Commerce Node browser calls, no unexpected 5xx, and no uncaught JS/.NET WASM errors.
- [x] Storefront Independence Boundary SIB8 targeted regression QA passed. 2026-07-25: solution build passed; focused Storefront/Control Plane boundary and auth tests passed 84/84; targeted host smoke for sign-in, logout, and maintenance passed 4/4; Playwright covered home, product detail, add-to-cart, cart update/remove, checkout start, login/logout, currency preference, and same-origin network assertions with no direct Commerce Node or Control Plane browser calls. Public response checks for home/product/cart did not expose provider secrets, internal settings, node credentials, Control Plane routes, or Commerce admin routes.
- [x] Storefront Presentation Foundation SPF15 production-facing QA passed. 2026-07-27: focused Presentation/V2/WASM/Starter builds passed; focused architecture/package/page/endpoint/WASM/Starter/StorefrontBuilder/commerce cutover tests passed 180/180; builder and foundation isolation gates passed; headless browser QA covered home, category, search noindex, product purchase, product 404, cart add/update/remove/clear, COD order/email/account order detail/receipt, sign-in/register/recovery/account profile/address/change-password, payment success/cancel, robots, sitemap, maintenance route, and same-origin network guardrails with no direct browser call to Commerce Node. SPF15 fixed checkout address update to preserve the authenticated customer session through Presentation/Runtime before placing COD orders.
- [x] Storefront Presentation Cutover SPF18 route shell QA passed. 2026-07-27: all Presentation route pages now pass state/context/metadata into `StorefrontPage`, while `StorefrontPage` owns SEO/head/status handling, structured data, non-ready rendering fragments, and maintenance auto-refresh. Source scan found no direct `StorefrontSeoHead`, `PageTitle`, `HeadContent`, or `StorefrontResponseHeaders` in Presentation route pages. Presentation, V2, and Starter builds passed; focused Storefront page/indexing/presentation tests passed `66/69` with 3 future cutover skips.
- [x] Storefront V2 Manual Client Retirement F1.25 browser COD/network regression passed. 2026-07-27: `run-v2-local.ps1 -StopExisting -NoOpenBrowser` started the local stack; `run-storefront-registration-policy-e2e.ps1 -Headless`, `run-storefront-email-recovery-e2e.ps1 -Headless`, and `run-storefront-order-email-e2e.ps1 -Headless -TimeoutSeconds 1200` passed. Evidence saved under `.gstack/qa-reports/registration-policy-e2e/result.json`, `.gstack/qa-reports/email-recovery-e2e/result.json`, and `.gstack/qa-reports/order-email-e2e/result.json`. Registration policy verified disabled/standard states and zero forbidden browser calls; email recovery verified known-user reset, unknown-user generic response, and zero 5xx; COD order/email verified order placement `ORD-20260727-AC16F60D`, account order list/detail/receipt screenshots, exactly one order email, SMTP outage retry, zero 5xx, and `retiredFlowCallCount=0`.
- [x] Storefront Presentation Cutover SPF19 route ownership lock passed. 2026-07-27: Presentation routing no longer accepts host route assemblies; `StorefrontPresentationRouteOptions` and `AddStorefrontPresentationRoutes(...)` were removed, while host `AddAdditionalAssemblies` remains only for visual component discovery. Source scan found no `^@page` directives in Storefront V2, Starter, or V2 WASM. StorefrontBuilder static validation now rejects generated visual `@page` directives. Presentation, V2, and Starter builds passed; focused page/Builder tests passed `70/70`, and presentation guardrails passed `18/20` with 2 future cutover skips.
- [x] Storefront Presentation Cutover SPF20 visual SEO/head cleanup passed. 2026-07-27: V2, Starter, and V2 WASM visual views no longer render `PageTitle`, `HeadContent`, or `StorefrontSeoHead`; payment pending and maintenance refresh metadata are emitted by Presentation `StorefrontSeoHead`. Remaining V2 `StorefrontResponseHeaders` usage is host middleware/local endpoint policy only. Presentation, V2 WASM, V2, and Starter builds passed; indexing/page/presentation guardrails passed `68/69` with 1 future skip; targeted payment pending and maintenance auto-refresh host smoke passed `2/2`. The full `StorefrontV2HostSmoke` filter exceeded the command timeout, so focused SEO/status smoke evidence was used for SPF20.
...
- [x] Storefront Browser Action Boundary F1.56 guardrail QA passed. 2026-07-28: V2 visual JavaScript no longer constructs product-selection/cart payloads or calls Presentation application commands; Presentation binders own descriptor reading, selection preview, add-to-cart, cart badge, and consent commands. Source guardrails now reject visual-host command invocation, command payload construction, product purchase business result interpretation, and direct Runtime/Client compile references. Browser proof `node scripts/qa/storefront-browser-action-boundary-proof.js` covers product descriptor preview, add-to-cart, cart badge, cart page, and no direct Commerce Node browser calls.
- [x] Storefront Browser Semantics Boundary F1.64 product projection QA passed. 2026-07-29: V2 renders selected option state from Presentation semantic events only; focused Playwright proof `node scripts/qa/storefront-browser-semantics-v2-proof.js` opened `/product/catalog-qa-t-shirt`, selected the Red/XL option, verified price, stock, image, SKU, and GTIN projection from semantic event values, and recorded zero direct Commerce Node browser calls. Guardrails also reject raw preview field reads in V2 visual scripts.
...
- [x] Component MVP route is not exposed through menu or sitemap. Evidence: H3 route/source scans and `StorefrontComponentMvpArchitectureTests` keep `/__qa/component-mvp` out of navigation, footer, sitemap, page template catalog, and public content catalog.
- [x] Component MVP route is noindex or QA-only. Evidence: Presentation route emits `RobotsIndex = false` and `RobotsFollow = false`; Storefront current-store and public-redirect middleware use explicit `IsArchitectureQaPath` policy for the internal `/__qa/*` namespace so the QA route remains deterministic.
- [x] Result counts are recorded for release review. Evidence: RawHtml, Hybrid, Rail, and Network Playwright phases passed on 2026-08-10 with zero console errors, zero page errors, zero direct Commerce browser calls, and zero credential leaks. H3.14 reran all four phases with `-RuntimeTimeoutSeconds 120 -NoBuild`; Network recorded `serverUiCircuit: 0`, `directCommerce: 0`, and `webSockets: 0`.
...
- [x] Execute Storefront Shared Platform V2F12 browser release gate after Runtime/Client/Components cutover. 2026-07-25: `Storefront.V2` browser QA placed real COD orders, verified account/order/email/recovery/registration/SEO/consent flows, and blocked direct Commerce Node browser traffic; final build and focused shared-platform tests passed.
