...
- [x] SCR10 Storefront Components Compatibility Removal docs/static QA passed. 2026-07-26: current architecture and StorefrontBuilder docs now state `Storefront.Components` only owns `Contracts`, `Headless`, and `Browser`; V2 owns cart, checkout, account, product, catalog, and deals markup/CSS/layout; Runtime compatibility aliases are removed. Focused final-boundary tests passed `144/144`.
- [ ] SCR12 browser release QA: verify home, catalog/category/search, product detail/gallery/purchase, add-to-cart, cart update/remove/clear, checkout COD place order, account profile/addresses/orders/order-detail/change-password routes, and recovery UI after compatibility removal.
- [ ] SCR12 browser console/static asset QA: verify no missing `storefrontWasmInterop.js`, no unexpected JS console/page errors, anti-forgery-backed same-origin browser actions still submit, and no direct Commerce Node browser calls.
