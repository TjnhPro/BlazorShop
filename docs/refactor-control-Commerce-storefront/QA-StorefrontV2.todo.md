...
- [x] Product detail purchase UI is a portable `ProductPurchasePanel` fed by page-owned variant/sellability snapshots; add-to-cart, selection preview, delivery, stock, quantity, and disabled hard-block markers remain present. 2026-07-23 Rendering Components Phase 6: focused Product/Catalog guardrail tests passed.
- [x] Category, search, deals, and new releases compose `ProductSummaryGrid` through `StorefrontProductSummaryMapper` instead of duplicating product summary card logic. 2026-07-23 Rendering Components Phase 6: focused Product/Catalog guardrail tests passed.
...
- [x] Headless Storefront Foundation F6 runtime/module boundary decision recorded. 2026-07-24: no `Storefront.Runtime` or `Storefront.Features.*` project was created because repeated code remains tied to Storefront V2 host/BFF ownership and there is still one real storefront consumer. Architecture docs keep generated API transport in `Storefront.Client`, portable UI in `Storefront.Components/Features/*`, and route/BFF/SEO/design/deployment in Storefront V2. `dotnet test BlazorShop.Tests.V2/BlazorShop.Tests.V2.csproj --no-restore --filter "FullyQualifiedName~Runtime|FullyQualifiedName~Architecture"` passed 79/79 after rerunning without parallel testhost locks. Known warnings: existing MessagePack NU1902/NU1903 and Browserslist.
