...
- [x] Product detail gallery is a portable `Storefront.Components/Features/Product/ProductGallery` fed by page-owned `ProductGalleryItem` snapshots, while the page still resolves product SEO/status and media fallback. 2026-07-23 Rendering Components Phase 6: Storefront V2 build passed and `StorefrontBrandingMarkupTests.ProductPage_RendersProductImageGalleryComponent` guards the component boundary. 2026-07-26 Storefront Components Compatibility Removal SCR4: shared `Features/Product/ProductGallery` is retired; Storefront V2 owns `StorefrontProductGallery` markup and still consumes shared `ProductGalleryItem`/headless gallery state.
