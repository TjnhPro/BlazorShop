...
| BFF endpoints | `StorefrontPresentationCartEndpoints`, `StorefrontPresentationCheckoutEndpoints`, `StorefrontPresentationAccountEndpoints`, `StorefrontConsentEndpoints`, `StorefrontPresentationAuthEndpoints`, `StorefrontMediaEndpoints`, `StorefrontSeoEndpoints`, plus `Program.cs` mappings | Presentation/V2 host owns same-origin browser endpoints, antiforgery/session/cookies, SEO, media, and SSR route composition. |
...
| Checkout/COD | Manual `StorefrontApiClient`, Presentation checkout BFF, Components local API | Runtime checkout/payment-method facades, Presentation owns guest checkout route, idempotency, redirect validation, noindex/private headers, and same-origin BFF | checkout host smoke slices, browser checkout baseline | High |
