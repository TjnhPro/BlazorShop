...
- [x] Starter renders store-specific favicon/icon metadata through Presentation `StorefrontIconHead` and does not depend on V2 `StorefrontBrandHead`.
...
- 2026-07-29 F1.72: Starter `MainLayout.razor` owns the cart badge descriptor directly, and StorefrontBuilder composition now only adds generated visual class styling around that existing descriptor. StorefrontBuilder static gate and focused composition tests passed after the Browser runtime cutover and enhanced-navigation browser proof.
- 2026-08-08 Icon Head Rendering: `StorefrontStarterHostSmokeTests` proves Starter root HTML keeps `css/starter.css`, renders configured `FaviconUrl` through Presentation `StorefrontIconHead`, does not render V2-only `bs-storefront-language`, and has no duplicate primary `rel="icon"`.
- 2026-08-08 Foundation Page Cleanup: Starter no longer registers, renders, or advertises dedicated `/new-releases` or `/todays-deals` collection pages; `starter-generation.contract.yaml` excludes both route declarations so generated storefront proofs do not inherit those deleted pages.
