...
- [x] ControlPlane slug lifecycle gateway remains behind the ControlPlane boundary at release gate. 2026-07-15 SEO Routing Slug Phase 12: no Web-to-CommerceNode browser/API path was introduced; CommerceNode slug lifecycle contract is guarded by `CommerceNodeAdminStoreOpenApiMetadataTests`.
...
| --- | --- | --- | --- | --- |
| 2026-07-15 | Codex | Layout Asset Foundation headed QA | Passed | Storefront V2 verified home/category/product/search/cart/checkout/auth pages; ControlPlane verified Dashboard, Stores, Products, Orders, Payment Methods, and Currencies after login. Static asset endpoint checks returned 200 and browser capture found 0 asset failures, console/page errors, or direct CommerceNode calls from ControlPlane Web. |
| 2026-07-17 | Codex | V2 local runner ControlPlane smoke | Passed | `run-v2-local.ps1 -StopExisting` started all V2 services. Playwright visible browser verified Dashboard, Products, Orders, Payment Methods, Currencies, Stores, Health, and Actions pages call `http://localhost:5280` through `wwwroot/appsettings.Development.json` and show no console errors. |
...
- [x] Active V2 build/test target is the canonical main solution plus `BlazorShop.Tests.V2`. 2026-07-22 legacy removal Phase 3: `dotnet build BlazorShop.sln --no-restore` and `dotnet test BlazorShop.Tests.V2/BlazorShop.Tests.V2.csproj --no-restore` are documented in architecture rules; the temporary V2 solution filter has been removed.
