...
- [x] Canonical Storefront OpenAPI contract is stored under `contracts/storefront/storefront.openapi.json`; test snapshots are breaking-change guardrails only. 2026-07-26 OpenAPI Client Hardening OCH7: Commerce Node runtime contract tests compare live Swagger to the canonical contract.
- [x] Storefront public configuration exposes machine-readable capability projection without removing backward-compatible flat feature flags. 2026-07-24 Headless Storefront Platform Foundation F2: `StorefrontContractMappingsRegressionTests.ToPublicConfigurationContract_ProjectsMachineReadableCapabilities` and `CommerceNodeStorefrontOpenApiContractTests` passed in focused 45/45 run.
- [x] Storefront expected error responses expose stable machine-readable codes for frontend control flow. 2026-07-24 Headless Storefront Platform Foundation F2: `StorefrontErrorCodes_CoverFrontendControlFlowCategories` and error response schema assertions passed in focused 45/45 run.
- [x] Storefront OpenAPI has an explicit breaking-change compatibility guard for removed paths, operation IDs, schemas, properties, enum values, response statuses, security schemes, type changes, and optional-to-required changes. 2026-07-24 Headless Storefront Platform Foundation F2: `StorefrontSwagger_BreakingChangeGuard_CurrentDocumentIsCompatibleWithBaseline` passed in focused 45/45 run.
- [x] Generated C# Storefront client compiles as a backend-independent project. 2026-07-24 Headless Storefront Platform Foundation F3: `BlazorShop.PresentationV2/BlazorShop.Storefront.Client` generated via pinned NSwag config; focused `OpenApi|GeneratedClient` run passed 53/53, including `StorefrontGeneratedClientFoundationTests` build/dependency/generated-source checks and strict TypeScript generation.
- [x] Storefront generated C# client regeneration gate is part of release validation. 2026-07-26 OpenAPI Client Hardening OCH4: `scripts/qa/run-storefront-client-regeneration-gate.ps1` runs canonical-contract generation and fails on generated-source drift before publishing `BlazorShop.Storefront.Client`.
- [x] Storefront package release checklist requires live Swagger versus canonical contract tests, generated-client regeneration gate, `Storefront.Client` build, and package consumer proof. 2026-07-26 OpenAPI Client Hardening OCH7.
