...
- [x] Browser runtime cutover proof remains part of release evidence. 2026-07-29 F1.72: `node scripts/qa/storefront-application-js-split-proof.js` and `node scripts/qa/storefront-browser-semantics-visual-proof.js` passed, covering same-origin browser commands, cart badge count events, enhanced navigation rebinding, duplicate-submit prevention, and product selection visual projection for price, stock, image, SKU, and GTIN.
