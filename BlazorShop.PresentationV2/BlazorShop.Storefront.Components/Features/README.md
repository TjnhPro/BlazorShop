...
`Features` is a temporary compatibility area for CSS-neutral compatibility wrappers while shared Storefront visual components are migrated to headless contracts and host-owned visual templates.
...
- These wrappers are not stable presentation contracts for Starter, StorefrontBuilder, or generated/custom storefronts.
- Starter, StorefrontBuilder, and generated/custom storefronts should consume `Contracts/{Capability}`, `Headless/{Capability}`, and `Browser` primitives instead of copying `Features` wrappers.
