namespace BlazorShop.Web.SharedV2.Helper
{
    using System.Net.Http.Headers;

    using BlazorShop.Web.SharedV2;
    using BlazorShop.Web.SharedV2.Helper.Contracts;

    public class HttpClientHelper : IHttpClientHelper
    {
        private readonly IHttpClientFactory _clientFactory;
        private readonly ITokenService _tokenService;

        public HttpClientHelper(IHttpClientFactory clientFactory, ITokenService tokenService)
        {
            _clientFactory = clientFactory;
            _tokenService = tokenService;
        }

        public HttpClient GetPublicClient()
        {
            return _clientFactory.CreateClient(HttpClientNames.Public);
        }

        public async Task<HttpClient> GetPrivateClientAsync()
        {
            var client = _clientFactory.CreateClient(HttpClientNames.Private);
            string token = await _tokenService.GetJwtTokenAsync(AuthStorageConstants.JwtTokenKey);

            if (string.IsNullOrEmpty(token))
            {
                return client;
            }

            client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
            return client;
        }
    }
}
