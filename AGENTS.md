...
- Commerce Node API must resolve Storefront route scope and Commerce Admin query scope into `StoreExecutionContext` at the Presentation boundary before services run. Infrastructure must not parse HTTP route values, query strings, headers, or hosts for normal store scope.
...
- Storefront Components.Ssr owns shared server-rendered semantic components over approved Components/Presentation contexts only. It must not use `@rendermode`, Browser/Runtime/Client/backend references, direct API calls, final V2 CSS classes, store-specific copy, CSS/assets, or generated storefront output; Presentation owns behavior/transport and V2 owns the registered visual wrapper, final values, and placement.
